...
export class Libdraw {
  ...
  public setClassColor(unitClass: string) {
    if (unitClass == "WARRIOR") return this.setColorRaw(0.78, 0.61, 0.43, 1);
    else if (unitClass == "WARLOCK") return this.setColorRaw(0.58, 0.51, 0.79, 1);
    else if (unitClass == "PALADIN") return this.setColorRaw(0.96, 0.55, 0.73, 1);
    else if (unitClass == "HUNTER") return this.setColorRaw(0.67, 0.83, 0.45, 1);
    else if (unitClass == "ROGUE") return this.setColorRaw(1.0, 0.96, 0.41, 1);
    else if (unitClass == "PRIEST") return this.setColorRaw(1.0, 1.0, 1.0, 1);
    else if (unitClass == "DEATHKNIGHT") return this.setColorRaw(0.78, 0.61, 0.43, 1);
    else if (unitClass == "SHAMAN") return this.setColorRaw(0.0, 0.44, 0.87, 1);
    else if (unitClass == "MAGE") return this.setColorRaw(0.41, 0.8, 0.94, 1);
    else if (unitClass == "MONK") return this.setColorRaw(0.33, 0.54, 0.52, 1);
    else if (unitClass == "DRUID") return this.setColorRaw(1.0, 0.49, 0.04, 1);
    else if (unitClass == "DEMONHUNTER") return this.setColorRaw(0.64, 0.19, 0.79, 1);
  ...
  public line(sx: number, sy: number, sz: number, ex: number, ey: number, ez: number) {
  ...
  public draw2DLine(sx: number, sy: number, ex: number, ey: number) {
    ...
    if (this.canvas.lines.length > 0) {
      buffer = table.remove(this.canvas.lines);
    ...
    L.line.SetColorTexture(this.m_line.r, this.m_line.g, this.m_line.b, this.m_line.a);
    L.Show();
  }
  ...
  public text(txt: string, font: any, x: number, y: number, z: number) {
    ...
    if (sx && sy) {
      ...
      F.SetText(txt);
      F.SetTextColor(this.m_line.r, this.m_line.g, this.m_line.g, this.m_line.a);
  ...
  public circle(x: number, y: number, z: number, size: number) {
    ...
    for (let v = 0; v < this.fullCircle; v += this.smallCircleStep * 10) {
      let ret = this.w2s(x + math.cos(v) * size, y + math.sin(v) * size, z);
      nx = ret[0];
      ny = ret[1];
  ...
  public groundCircle(x: number, y: number, z: number, size: number) {
    ...
    for (let v = 0; v < this.fullCircle; v += this.smallCircleStep * 8) {
      ...
      fz ? fz : (fz = z);
      let ret2 = this.w2s(fx + math.cos(v) * size, fy + math.sin(v) * size, fz);
      nx = ret2[0];
      ny = ret2[1];
      if (lx && ly) this.draw2DLine(lx, ly, nx, ny);
      lx = nx;
  ...
  clearCanvas() {
    ...
    if (this.canvas.texturesUsed.length > 0) {
      for (const i of forRange(this.canvas.texturesUsed.length, 1, -1)) {
        ...
        table.insert(this.canvas.textures, table.remove(this.canvas.texturesUsed));
      }
    }

    //fonts
    if (this.canvas.fontStringsUsed.length > 0) {
      for (const i of forRange(this.canvas.fontStringsUsed.length, 1, -1)) {
        this.canvas.fontStringsUsed[i - 1].Hide();
        table.insert(this.canvas.fontStrings, table.remove(this.canvas.fontStringsUsed));
      }
    }
    ...
    if (this.canvas.linesUsed.length > 0) {
      for (const i of forRange(this.canvas.linesUsed.length, 1, -1)) {
        ...
        table.insert(this.canvas.lines, table.remove(this.canvas.linesUsed));
      }
    }
  }
